...
export default function ForgotPasswordPage() {
  ...
  const [userId, setUserId] = useState(null); // Add if you're tracking it
  const navigate = useNavigate();

  useEffect(() => {
    if (userId) {
      socket.emit('join-room', userId);
      socket.on('reset-approved', (data) => {
        navigate(data.link); // Redirect to the reset password page
      });
    }

    return () => {
      socket.off('reset-approved');
    };
  }, [userId, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setUserId(data.userId); // Make sure your API sends this back
    } catch (err) {
